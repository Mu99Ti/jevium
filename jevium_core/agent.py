...
def confidence_floor():
    """JEVIUM_MIN_CONFIDENCE; empty/invalid falls back to the conservative default. 0 disables."""
    try:
        return float(os.environ.get("JEVIUM_MIN_CONFIDENCE", "0.3") or "0.3")
    except ValueError:
        return 0.3


class Agent:
    def __init__(self, url, goals, *, record_dir=None, screenshots=False, browser_factory=None, max_steps=None):
        ...
        self.browser = browser_factory(url) if browser_factory else Browser(url)
    ...
    def command(self, name, body=None):
        ...
        if name == "tick":
            ...
            if selected in {"DONE", "BLOCKED"}:
                ...
                state["elapsed_ms"] = round((time.perf_counter() - state["started_at"]) * 1000)
                return self.snapshot()
            if selected == "NEEDS_HUMAN":
                state["status"] = "waiting_human"
                state["waiting"] = {
                    "trigger": "model",
                    "reason": GATE_REASONS["model"],
            ...
            if state["record"]:
                ...
                with (self.record_dir / "steps.jsonl").open("a") as fh:
                    fh.write(json.dumps(state["history"][-1]) + "\n")
        ...
        while True:
            ...
            if status == "waiting_human":
                if on_waiting is not None:
                    on_waiting(dict(self.state["waiting"] or {}))
                    self._resume.clear()
                else:
                    self._resume.wait()
                self._after_resume()
